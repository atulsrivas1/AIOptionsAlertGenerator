...
from src.quant_lab.data_loader import get_historical_stock_data_alpha_vantage, get_historical_option_data
...
time.sleep(15) # Wait for the MLflow server to start
...
def train_and_predict(train_data: pd.DataFrame, test_data: pd.DataFrame, triple_barrier_params: dict):
    ...
    feature_columns = [
        ...
        'gex_imbalance', 'dynamic_gex', 'gex_concentration', 'gex_momentum',
        'implied_vol_std', 'implied_vol_change',
        'total_uoa_score', 'total_uoa_direction',
        'expected_flow', 'flow_concentration', 'flow_asymmetry', 'flow_pressure',
        'term_structure_slope', 'term_structure_intercept', 'term_structure_r_squared', 'term_structure_curvature',
        'atm_volatility', 'put_skew', 'call_skew', 'skew_asymmetry', 'skew_slope',
        'vol_of_vol_all', 'vol_of_vol_short_term', 'vol_of_vol_long_term',
        'skew_curvature', 'surface_stability'
    ]
    ...
    # Ensure all three classes are present in the training data for this fold
    if len(y_train_mapped.unique()) < 3:
        print(f"  WARNING: Skipping fold. Not all 3 classes present in training data.")
        # Return empty signals and importances for this fold
        return pd.Series(dtype='int64'), pd.Series(dtype='float64')
...
if __name__ == "__main__":
    ...
    with mlflow.start_run(run_name="Long-Term Strategy"):
        # --- Configuration ---
        ...
        TRIPLE_BARRIER_PARAMS = {
            "profit_target": 0.08, # Adjusted for longer horizon
            "stop_loss": 0.04, # Adjusted for longer horizon
            "time_horizon": 30, # Long-term horizon
        ...
        # Load underlying stock data from Alpha Vantage
        underlying_data = get_historical_stock_data_alpha_vantage(UNDERLYING_TICKER, START_DATE, END_DATE)
        if underlying_data.empty:
            print("Error: Could not load underlying stock data. Exiting.")
            exit()
        # Load option data from Polygon.io
        option_data = get_historical_option_data(OPTION_TICKER, START_DATE, END_DATE)
        if option_data.empty:
            print("Warning: Could not load option data. Proceeding with underlying data only.")
        # Combine data sources
        raw_data = underlying_data.join(option_data, lsuffix='_underlying', rsuffix='_option')
        raw_data['close'] = raw_data['close_underlying'] # Use underlying close for labeling
        raw_data['implied_volatility'] = raw_data['implied_volatility']
